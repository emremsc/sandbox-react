...
export function TooltipGraph(props: { animate?: boolean } = { animate: true }) {
  const id1 = React.useId();
  const id2 = React.useId();

  const root = React.useRef<HTMLDivElement>(null);
  const dot = React.useRef<HTMLDivElement>(null);
  const gradient = React.useRef<SVGPathElement>(null);

  const [offsetDistance, setOffsetDistance] = React.useState(100);
  const offsetD = useSpring(100, { damping: 50, stiffness: 550 });
  const x = useMotionValue(363);
  const xSpring = useSpring(363, { damping: 50, stiffness: 350 });
  ...
  const price = React.useMemo(() => {
    ...
    const calculatedPrice = basePrice + offsetDistance * pricePerUnit;
  ...
  function animate(distance: number) {
    ...
    const offsetLeft = distance - left;
    const offsetDistance = clamp((offsetLeft / parentWidth) * 100, [0, 100]);
    setOffsetDistance(offsetDistance);
    offsetD.jump(offsetDistance);

    setTimeout(() => {
      const { x: dotX, width: dotWidth } = dot.current!.getBoundingClientRect();
      const xFromParent =
        (dotX - parentX + dotWidth / 2 - lineWidth / 2) / scale;
      x.set(xFromParent);
      xSpring.jump(xFromParent);

      gradient.current!.style.setProperty(
        "clip-path",
        `inset(0 ${parentWidth / scale - xFromParent - lineWidth * 2}px 0 0)`
      );
    }, 1);
  }
  ...
  return (
    <motion.div
      onPan={(_, info) => {
        animate(info.point.x);
      }}
      className="w-full h-full flex-center touch-pan-y p-16"
    >
      <div
        className="relative max-sm:scale-(--mobile-scale)"
        ref={root}
        style={
          {
      ...
      >
        <motion.div
          className="h-[calc(100%_-_10px)] rounded-full bg-gray8 absolute"
          style={{
            width: lineWidth,
            x: xSpring,
          }}
        >
          <div
            className="rounded-full leading-(--height) h-(--height) bg-gray5 px-2 absolute translate-center-x text-13 text-gray12 whitespace-nowrap flex-center"
            style={
              {
                "--height": "24px",
              } as React.CSSProperties
        ...
        <motion.div
          className="w-3 h-3 absolute bg-orange rounded-full"
          ref={dot}
          style={{
            offsetPath: "path(var(--d))",
            boxShadow: "0 0 0 2px #FFF, 0 0 8px 2px rgba(0, 0, 0, 0.12)",
            // offsetDistance: `${offsetDistance}%`,
            offsetDistance: useMotionTemplate`${offsetD}%`,
          }}
        />
        <svg width="366" height="213" viewBox="0 0 366 213" fill="none">
          <path d={gradientDefinition} fill={`url(#${id2})`} />
          <path ref={gradient} d={gradientDefinition} fill={`url(#${id1})`} />
          <g strokeWidth="2.2" strokeLinecap="round">
            <path d={strokeDefinition} stroke="var(--color-gray8)" />
            <path
              d={strokeDefinition}
              stroke="var(--color-orange)"
              pathLength={1}
              style={{
                strokeDashoffset: 1 - offsetDistance / 100,
                strokeDasharray: 1,
                opacity: offsetDistance === 0 ? 0 : 1,
              }}
            />
          </g>
          <defs>
            ...
            >
              <stop stopColor="var(--color-orange-bg)" />
              <stop
                offset="1"
                stopColor="var(--color-orange-bg)"
                stopOpacity="0"
              />
            </linearGradient>
            <linearGradient
              ...
              gradientUnits="userSpaceOnUse"
            >
              <stop stopColor="var(--color-gray5)" />
              <stop offset="1" stopColor="var(--color-gray5)" stopOpacity="0" />
            </linearGradient>
          </defs>
        </svg>
      </div>
