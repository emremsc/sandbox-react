...
export default function PerspectiveTunnel({ squareCount = 16, perspective = 800 }: { squareCount?: number; perspective?: number }) {
  ...
  return (
    <div ref={containerRef} className='size-full grid place-items-center relative' style={{ perspective: `${perspective}px` }}>
      {squares.map((square) => (
        <div
          key={square.index}
          className='absolute w-96 h-96 border-4 border-black/80 rounded-3xl'
          style={{
